    ...
    public class TransactionController : BaseController
    {
        ...
        public async Task<IActionResult> Withdraw(string modelJson)
        {
            try
            {
                ...
                //                        $"Your balance TikTok (*****) is not enough."));
                //}

                if (model.Amount < 20000)
                {
                    return new OkObjectResult(new GenericResult(false,
                        "Withraw ***** (*****) amount minimum is 20,000*****"));
                }
                ...
                {
                    return new OkObjectResult(new GenericResult(false,
                        "Withraw TikTok (*****) maximum is 10000000***** on day"));
                }
                ...
                var transactionReceipt = await _tronService.EasyTransferAssetByPrivate(
                     ...
                     CommonConstants.TRONELCContract, balanceTTTransfer);
                ...
                {
                    return new OkObjectResult(new GenericResult(false, "There was a problem with the transfer ***** (*****) to address receiving."));
                }
        ...
        public async Task<IActionResult> Buy(string modelJson)
        {
            try
            {
                ...
                if (transactionReceipt.success == true)
                {
                    ...
                    {
                        return new OkObjectResult(new GenericResult(false, $"There was a problem with the transfer {ttReceivingAmount + bonus}***** to Wallet ***** *****"));
                    }
                }
                else
                {
                ...
                return new OkObjectResult(new GenericResult(true,
                    $"Buy {ttReceivingAmount + (ttReceivingAmount * 0.1M)}***** to Wallet TikTok ***** is success."));
            }
            catch (Exception ex)
            {
                return new OkObjectResult(new GenericResult(false, ex.Message));
